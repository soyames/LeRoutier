...
export default function App() {
  ...
  const screens = { trips: <Trips />, tickets: <Tickets focusId={focusId} />, stations: <Stations />, parcels: <Parcels />, tracking: <Tracking />, account: <Account />, checkout: <Checkout /> };
  ...
  // The checkout controls its own auth timing: the sign-in panel appears only
  // after "Continuer vers le paiement", never before.
  return <AppShell online={online} role="Voyageur" title={titles[page]} subtitle="LeRoutier Bénin" nav={navWithOnboarding} active={page} onNavigate={id => navigate('/' + id)}>{!['checkout','trips'].includes(page) && <SessionPanel />}{screens[page]}</AppShell>;
}
