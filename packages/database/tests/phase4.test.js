...
test('reopenable credentials stay private; scanner hashes and notification payloads contain no bearer',async()=>{
  const {b,t}=await issued();assert.ok(t.token.startsWith('LRT1.'));assert.ok(!t.token.includes(b.id));
  const stored=await db.transaction(async tx=>(await tx.query('SELECT * FROM ticket_credentials WHERE booking_id=$1',[b.id])).rows[0]);
  assert.equal(stored.token,t.token);assert.equal(stored.manual_code,t.manualCode);
  assert.notEqual(stored.token_hash,t.token);assert.notEqual(stored.code_hash,t.manualCode);
  const outbox=await db.transaction(async tx=>(await tx.query('SELECT payload FROM outbox WHERE aggregate_id=$1',[b.id])).rows);
  assert.equal(JSON.stringify(outbox).includes(t.token),false);assert.equal(JSON.stringify(outbox).includes(t.manualCode),false);
  await assert.rejects(ticket.issue({...passenger,id:randomUUID()},b.id),{code:'FORBIDDEN'});
});
...
test('reopening a ticket preserves printed credentials',async()=>{const {b,t}=await issued(),next=await ticket.issue(passenger,b.id);assert.equal(next.version,t.version);assert.equal(next.token,t.token);assert.equal(next.manualCode,t.manualCode);assert.equal((await verify(t)).valid,true);assert.equal((await verify(next)).valid,true);});
...
test('cancelled and unpaid bookings cannot produce valid QR',async()=>{const unpaid=await hold();await assert.rejects(ticket.issue(passenger,unpaid.id));const {b,t}=await issued();await d.transition(passenger,b.id,'cancel');await assert.rejects(verify(t));const archive=await ticket.issue(passenger,b.id);assert.equal(archive.token,t.token);assert.equal(archive.manualCode,t.manualCode);assert.equal(archive.validForBoarding,false);await assert.rejects(verify(archive));});
