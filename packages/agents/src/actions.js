...
const validate = (shape, input) => {
  invariant(input && typeof input === 'object' && !Array.isArray(input) &&
    Object.keys(input).every(k => Object.hasOwn(shape, k)),
  'INVALID_ACTION_INPUT', 'Action input does not match its schema.');
  for (const [key, check] of Object.entries(shape)) {
    if (input[key] === undefined) { invariant(check.optional === true, 'INVALID_ACTION_INPUT', 'A required input field is missing.'); continue; }
    check(input[key]);
  }
  return input;
};
...
export function createActions(ctx) {
  const { db, domain, payments, payouts, recovery } = ctx;
