...
function StorageCard({storage}){
  const armed=storage.storageProtection==='armed';
  const tone=!armed?'warning':storage.registrationsOpen?'success':'warning';
  const label=!armed?'Protection non armée':storage.registrationsOpen?'Inscriptions ouvertes':'Inscriptions suspendues';
  return <Card className="stack">
    ...
    {armed
      ? <p className="small muted">Seuil d’arrêt des nouvelles inscriptions : {storage.registrationStopPercent} %.</p>
      : <p role="alert">Aucune limite de stockage n’est configurée : le seuil de {storage.registrationStopPercent} % ne peut pas se déclencher
