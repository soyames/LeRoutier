...
export function ticketDocument(ticket, kind = 'ticket') {
  ...
  return {
    ...
    reference: ref, number: `${kind === 'invoice' ? 'LRF' : kind === 'cancellation' ? 'LRA' : 'LRB'}-${ref}`,
    ...
    sections: [
      ...
      { title: kind === 'cancellation' ? 'Situation du remboursement' : 'Paiement', rows: [...paymentRows,
        ...
        ...b.payments.map(p => ['Paiement ' + p.id.toUpperCase(), `${fcfa(p.amount_minor)} · ${p.status === 'refunded' ? 'Remboursé' : 'Reçu'} · ${when(p.created_at)}`])] },
    ],
    qr: kind === 'ticket' ? ticket.token : null, manualCode: kind === 'ticket' ? ticket.manualCode : null,
    qrHelp: 'À présenter à l’embarquement',
    notes: kind === 'ticket' ? [...(!ticket.validForBoarding ? ['Document de réservation — ne permet pas un nouvel embarquement.'] : []), ...travelNotes]
    ...
    help: 'Aide : leroutier.app/about · Communiquez votre référence de réservation.',
  };
}
